...
class App:
    def __init__(self, thresholds, flip_frame = False):
        ...
        self.FEEDBACK_ID_MAP = {
                                ...
                                2: ('Error Detected: KNEE OVER TOE', 170, (255, 80, 80)),
                                3: ('Error Detected: SQUAT TOO LOW', 125, (255, 80, 80))
                               }
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            # Camera is aligned properly.
                ...
                draw_dotted_line(frame, hip_coord, start=hip_coord[1]-80, end=hip_coord[1]+20, line_color=self.COLORS['white'])
                ...
                draw_dotted_line(frame, knee_coord, start=knee_coord[1]-50, end=knee_coord[1]+20, line_color=self.COLORS['white'])
                ...
                draw_dotted_line(frame, ankle_coord, start=ankle_coord[1]-50, end=ankle_coord[1]+20, line_color=self.COLORS['white'])

                # ------------------------------------------------------------
        
                
                # Join landmarks.
                cv2.line(frame, shldr_coord, elbow_coord, self.COLORS['magenta'], 4, lineType=self.linetype)
                cv2.line(frame, wrist_coord, elbow_coord, self.COLORS['magenta'], 4, lineType=self.linetype)
                cv2.line(frame, shldr_coord, hip_coord, self.COLORS['magenta'], 4, lineType=self.linetype)
                cv2.line(frame, knee_coord, hip_coord, self.COLORS['magenta'], 4,  lineType=self.linetype)
                cv2.line(frame, ankle_coord, knee_coord,self.COLORS['magenta'], 4,  lineType=self.linetype)
                cv2.line(frame, ankle_coord, foot_coord, self.COLORS['magenta'], 4,  lineType=self.linetype)
                ...
                if current_state == 's1':
                    ...
                    if len(self.state_tracker['state_seq']) == 3 and not self.state_tracker['INCORRECT_POSTURE']:
                        ...
                        #send frames to model for prediction
                        prediction = self.cnn_model.predict(frame)
                        #if rep is correct, increase counter
                        if prediction == 'gf':
