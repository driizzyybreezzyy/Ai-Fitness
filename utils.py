...
def find_angle(p1, p2, ref_pt=np.array([0,0])):
    # Subtract the reference point from p1 to get the vector from the reference point to p1
    p1_ref = p1 - ref_pt
    
    # Subtract the reference point from p2 to get the vector from the reference point to p2
    p2_ref = p2 - ref_pt

    # Compute the cosine of the angle between the vectors using dot product and normalization
    cos_theta = (np.dot(p1_ref, p2_ref)) / (1.0 * np.linalg.norm(p1_ref) * np.linalg.norm(p2_ref))
    
    # Clip the cosine value to ensure it's within the valid range [-1, 1] to avoid errors with arccosine function
    cos_theta = np.clip(cos_theta, -1.0, 1.0)

    # Calculate the angle theta in radians using the arccosine function
